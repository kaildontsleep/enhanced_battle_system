...
function checkSuperAdminAccess() {
    ...
    console.log('isAdmin:', window.allianceSystem.isAdmin);
    console.log('currentUser:', window.allianceSystem.currentUser);
    ...
    // 修复：统一使用 'superadmin' 角色名称
    if (!window.allianceSystem.currentUser || window.allianceSystem.currentUser.role !== 'superadmin') {
        console.log('Current user role:', window.allianceSystem.currentUser?.role);
...
async function loadAdminsData() {
    try {
        ...
        if (adminsList) {
            adminsList.innerHTML = '<div class="no-data">加载失败</div>';
        }
    }
}
...
async function loadAuditLog() {
    try {
        ...
        if (auditLog) {
            auditLog.innerHTML = '<div class="no-data">加载失败</div>';
        }
    }
}
...
function updateStats() {
    ...
    if (todayActionsEl) todayActionsEl.textContent = todayActions;
}
